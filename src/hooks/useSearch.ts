...
const getSearchConfig = ({ originId, destinationId, date }: SearchParams) => ({
...
export function useSearch({ originId, destinationId, date }: SearchParams) {
  ...
  const { data, fetchStatus, refetch } = useQuery(
    getSearchConfig({ originId, destinationId, date })
  );

  return { journeys: data, fetchStatus, search: refetch };
}
